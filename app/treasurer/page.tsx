...
function TreasurerReportsContent() {
  ...
  const handleDeleteReport = async (id: number, fileUrl: string) => {
    if (confirm('Delete this treasurer report? This action cannot be undone.')) {
      try {
        ...
        if (fileName) {
          await supabase.storage.from('treasurer-reports').remove([fileName]);
        }
        ...
        {showAddForm && (
          <div className="bg-gray-800/80 rounded-xl border border-gray-700 p-4 sm:p-6 mb-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-gray-400 text-xs sm:text-sm block mb-1">Report Date</label>
                <input
                  type="date"
        ...
        <div className="space-y-3 sm:space-y-4">
          {error && (
            <div className="bg-red-900/30 border border-red-700 rounded-xl p-4 text-center">
              ...
              <button onClick={loadReports} className="mt-2 text-xs text-red-400 hover:text-red-300">
                Try Again
              </button>
            </div>
          )}
              ...
              <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-800">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-500 text-xs sm:text-sm mt-1">{formatDate(report.date)}</p>
                  </div>
...
export default function TreasurerPage() {
  return (
    <PasswordProtect pageName="Treasurer's Reports">
      <TreasurerReportsContent />
    </PasswordProtect>
  );
}